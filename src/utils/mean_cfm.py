...
def get_full_velocity_field_from_loader(
        t, xt, train_loader, sigmamin=.0, n_samples=None, flatten=True):
    """
    Compute the closed-form velocity field u* using a stream of x1' samples
    from a data loader. This uses a numerically stable one-pass log-sum-exp
    accumulation and does not require storing all x1' in memory.

    Parameters
    ----------
    t: (batch_size, 1)
    xt: (batch_size, C, H, W) or flattened (batch_size, D)
    train_loader: DataLoader yielding (x1, y)
    sigmamin: float
    n_samples: int | None, number of samples to use from the loader
    flatten: bool
    """
    if flatten:
        sizes = xt[0].shape
    ...
    sigmat = (1 - (1 - sigmamin) * t)  # (B,1)

    # Streaming log-sum-exp for softmax weights across all x1' samples.
    m = torch.full((xt.shape[0],), float("-inf"), device=device)
    s = torch.zeros((xt.shape[0],), device=device)
    w = torch.zeros((xt.shape[0], xt.shape[1]), device=device)

    seen = 0
    for batch in train_loader:
        x1prime = batch[0].to(device)
        if n_samples is not None:
            remaining = n_samples - seen
            if remaining <= 0:
                break
            if x1prime.shape[0] > remaining:
                x1prime = x1prime[:remaining]
        ...
        arg_softmax = -((mut - xt[None, :, :]) ** 2).sum(dim=-1)
        arg_softmax /= 2 * (sigmat ** 2).T  # (1,B)

        # ucond: (N, B, D)
        ucond = (x1prime[:, None, :] - (1 - sigmamin) * xt[None, :, :]) / sigmat.T[:, :, None]

        # Streaming update
        chunk_max = torch.max(arg_softmax, dim=0).values
        new_m = torch.maximum(m, chunk_max)
        scale_old = torch.exp(m - new_m)
        scale_chunk = torch.exp(arg_softmax - new_m)

        s = s * scale_old + scale_chunk.sum(dim=0)
        w = w * scale_old[:, None] + (scale_chunk[:, :, None] * ucond).sum(dim=0)
        m = new_m

        seen += x1prime.shape[0]
